...
export default function DirectPlacedOrders() {
    ...
    const [directOrdersForSearch, setDirectOrdersForSearch] = useState([]);
    ...
    function searchDirectOrders(orderId) {
        orderId = orderId.toUpperCase();
        setDirectOrders(directOrdersForSearch);

        if (orderId !== '') {
            const filteredOrders = directOrdersForSearch.filter(order => order.pOrderId.includes(orderId));
            setDirectOrders(filteredOrders);
        }
    }
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:8070/procurement/direct/')
            .then((res) => {
    ...
    return (
        <div>
            <div className="row" style={{ height: "100%" }}>
                {/* Left tab pane */}
                <div id="procurement-direct-orders-tab-pane" className="col-3" style={{ backgroundColor: "#b9bdba" }}>
                    <div
                        id="procurement-direct-orders-label"
                        style={{
                    ...
                    >
                        <b>{ProcurementConstants.DIRECT_PLACED_ORDERS_PAGE_TITLE}</b>
                    </div>
                    <div id="procurement-direct-orders-profile-details" style={{ textAlign: "center", marginTop: "3%" }}>
                        <b>{employeeName}</b>
                        <br />
                        {ProcurementConstants.PROCUREMENT_STAFF_USER_ROLE}
                    </div>
                    <div id="procurement-direct-orders-tabs" style={{ marginTop: "8%", fontSize: "150%", marginLeft: "10%" }}>
                        <div id="procurement-direct-orders-tabs-1" style={{ marginBottom: "30px" }}>
                        ...
                        <div id="procurement-direct-orders-tabs-2" style={{ marginBottom: "30px" }}>
                        ...
                        <div id="procurement-direct-orders-tabs-3" style={{ marginBottom: "30px" }}>
                        ...
                        <div id="procurement-direct-orders-tabs-4" style={{ marginBottom: "30px" }}>
                            <a href="/procurementstaffhome/directorders/" style={{ textDecoration: "none" }}>
                                ...
                                />
                                <b style={{ color: "#3a7ae0" }}>{ProcurementConstants.NAVIGATION_TAB4_NAME}</b>
                            </a>
                        </div>
                    </div>
                </div>
                <div id="procurement-direct-orders-content" className="col" style={{ marginTop: "2%" }}>
                    <div id="procurement-direct-orders-top">
                        <div id="procurement-direct-orders-logout-button">
                            <a
                                href="/"
                                style={{ float: "right" }}
                                onClick={() => {
                        ...
                        <div id="procurement-direct-orders-date-and-time"><b style={{ marginLeft: "10%" }}>{currTime.toLocaleTimeString()}</b>
                            <span style={{ marginLeft: "5%" }}>
                                {currTime.toLocaleDateString("en-US", dateFormatOptions)}
                            </span>
                        </div>
                    </div>
                    <div id="procurement-direct-orders-content-body" style={{ marginTop: "3%" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            <h2><b>{ProcurementConstants.DIRECT_PLACED_ORDERS_PAGE_PAGE_BODY_HEADING}</b></h2>
                            <div className="form-outline" style={{ marginRight: '65px', position: 'relative', display: 'flex' }}>
                                <button type="submit" className="search-button">
                                    <i className="fa fa-search"></i>
                                </button><input type="search" id="form1" className="form-control" placeholder="Search via PO ID" maxLength={7} onChange={(e) => { searchDirectOrders(e.target.value) }} />
                            </div>
                        </div>
