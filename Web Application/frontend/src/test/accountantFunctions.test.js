...
describe("Testing specific purchase order", () => {
  it("Positive test case", async () => {
    ...
    expect(res.data[0].invoiceId).toBe(purchaseOrder.invoiceId);
    expect(res.data[0].deliveryId).toBe(purchaseOrder.deliveryId);
    expect(res.data[0].pOrderId).toBe(purchaseOrder.pOrderId);
  });
});
