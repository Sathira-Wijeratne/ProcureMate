...
class DeliveryNoteHistoryScreen extends StatefulWidget {
  const DeliveryNoteHistoryScreen(this._width, this._height, this.user,
      {super.key});
  ...
  @override
  State<DeliveryNoteHistoryScreen> createState() =>
      _DeliveryNoteHistoryScreenState();
}
...
class _DeliveryNoteHistoryScreenState extends State<DeliveryNoteHistoryScreen> {
  ...
  void filter(String option) {
    setState(() {
      POHistory2 =
          POHistory1.where((element) => element['status'] == option).toList();
    });
  }

  Future<void> getConfirmedDeliveryNotes() async {
    POHistory1 = await DBService.getAllApprovedDeliveryNotes(widget.user.empId);
    setState(() {
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: SizedBox(
              child: ListView(
                  children: POHistory2.map((e) => GestureDetector(
                        onTap: () {
                          ...
                          Navigator.of(context).push(
                            MaterialPageRoute(
                              builder: (context) =>
                                  ViewDeliveryNoteInDetailScreen(widget._width,
                                      widget._height, widget.user, e, false),
                            ),
                          );
                        },
                        child: Card(
